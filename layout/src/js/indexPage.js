// $('.wrapper__top-slider').removeClass('top-slider--no-js');
$('.slider__block').slick({
  infinite: true,
  slidesToShow: 1,
  slidesToScroll: 1,
...
// кастомный select
const renderSelect = function(selectClass, selectNumber) {
...
function init() {
  ...
  if (window.matchMedia("(max-width: 1500px)").matches) {
    myMap.setCenter([48.219000, 104.268000])
  };
...
$("a[href^='#']").click(function() {
  ...
  if ($(this).attr("href") == "#calculate-cost") {
    goto.find('input[name="name"]').focus()
  }
  // return false;
});
...
const selectItem = $('.size__select--first').find('.size--select__item');
